...
public class CuentaDAOImpl implements CuentaDAO {
	...
	public List<Cuenta> listarCuentas() throws Exception {
		...
		try (Connection conn = Conexion.getConexion();
			...
			while (rs.next()) {
				...
				// cuenta.setTipoCuenta(rs.getInt("TipoCuenta_Cu"));No sirve porque guarda el
				// numero
	...
	public List<Cuenta>  CargarDDlCuentas(String dni) {
		...
		try (Connection conn = Conexion.getConexion()) {
			PreparedStatement stmt = conn.prepareStatement("select cuentas.SALDO_Cu, cuentas.NumeroCuenta_Cu, cuentas.ClienteDNI_Cu, cuentas.TipoCuenta_Cu, cuentas.CBU_Cu , t.NombreTipo_Tc AS NombreTipoCuenta from cuentas inner join TiposCuentas t on cuentas.TipoCuenta_Cu =t.IdTipoCuenta_Tc where cuentas.Estado_Cu= 1 and cuentas.ClienteDNI_Cu= ?");
