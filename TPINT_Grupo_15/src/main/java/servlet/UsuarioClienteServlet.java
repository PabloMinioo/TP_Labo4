...
public class UsuarioClienteServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			session.setAttribute("listaCuentas", cuentas);// le pongo session y no request porque sino cada vez que se
															// actulaiza lapagina se pierden nlos DDL precargados
...
//parseo
		...
		float importe = Float.parseFloat(importeStr); /// El problema es quemum eustra con 1 decimal, ver is estan de
														/// acuerdo!!!
...
//agarro los valores de mi jsp
		...
		BigDecimal importe = new BigDecimal(importeStr).setScale(2, RoundingMode.HALF_UP);/// no uso float por las
																							/// dudas, yaque usea arriba
																							/// y hay problemas e
																							/// redondeo
		...
		try {
			...
			if (solicitud) {
				...
				// "/vistas/ConfirmacionPrestamo.jsp");
